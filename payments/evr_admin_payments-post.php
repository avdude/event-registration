...
function evr_admin_payment_post(){
    global $wpdb, $company_options;
                ...
                if (isset($_REQUEST['payer_status'])){
                ...
                if (isset($_REQUEST['payment_status'])){
				    ...
				    $payment_status = "manually posted";
   					}
                ...
                if (isset($_REQUEST['mc_currency'])){
				    ...
				    $mc_currency =$company_options['default_currency'];
     ...
     $sql_data = array('%s','%s','%s','%s','%s','%s','%s','%s','%s',
            ...
            if ($wpdb->insert( get_option('evr_payment'), $sql, $sql_data )){ ?>
                ...
                <?php }else { ?>
        		<div id="message" class="error"><p><strong><?php _e('There was an error in your submission, please try again. The payment was not saved!','evr_language');?><?php print $wpdb->last_error;?>.</strong></p>
 		...
 		if ($send_payment_rec == "Y") {	
					...
					$message = html_entity_decode(nl2br($company_options['payment_message']));
                       ...
                       $ReplaceValues = array($payment_dtl['payer_id'], $payment_dtl['first_name'], $payment_dtl['last_name'],
                       $payment_dtl['payer_email'], stripslashes($event_dtl->event_name), $company_options['company_email'], 
                       $payment_link, $payment_dtl['mc_gross'], $payment_dtl['txn_id'], $payment_dtl['txn_type'], 
                       $payment_dtl['address_street'], $payment_dtl['address_city'], $payment_dtl['address_state'], 
                       $payment_dtl['address_zip'], $payment_dtl['address_country'], $event_dtl->start_date, 
                    ...
                    $headers .= 'From: "' . stripslashes($company_options['company_name']) . '" <' . $company_options['company_email'] . ">\r\n";
                    wp_mail($attendee_dtl->email,  stripslashes($subject), html_entity_decode($email_body), $headers);
                   ?>
